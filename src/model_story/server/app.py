...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info("model-story server starting (db=%s)", settings.db)
    yield
    logger.info("model-story server shutting down")
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    from model_story.server.routes_runs import router as runs_router
    from model_story.server.routes_snapshot import router as snapshot_router
    from model_story.server.sse import router as sse_router
    from model_story.server.ws import router as ws_router
