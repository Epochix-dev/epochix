...
primary metric name, and lower-is-better flag — via the ``model_story.tasks``
entry-point group.
...
conforming to the same structure::
    ...
    # my_pack/task.py
    from model_story.plugins.task_registry import TaskDefinition
...
def _load_task_plugins() -> None:
    ...
    eps = importlib.metadata.entry_points(group="model_story.tasks")
    for ep in eps:
        try:
            value = ep.load()
