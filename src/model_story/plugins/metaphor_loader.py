...
Third-party packages provide domain-specific :class:`~model_story.models.MetaphorCard`
templates by pointing to a YAML file via the ``model_story.metaphor_packs`` entry-point
group.

YAML schema::

    task: biometric          # task type key (must match model_story.enums.TaskType)
    cards:
      awakening:
...
from model_story.models import MetaphorCard
...
def _load_metaphor_plugins() -> None:
    ...
    eps = importlib.metadata.entry_points(group="model_story.metaphor_packs")
    for ep in eps:
        try:
            value = ep.load()
