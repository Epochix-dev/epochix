...
converts the scalar summaries into model-story :class:`~model_story.models.MetricEvent`
...
Usage::
    ...
    # Python
    from model_story.integrations.tensorboard_import import import_tensorboard
...
def import_tensorboard(
    ...
    *,
    ...
    port:
        Port of the model-story server to push events to.
    open_browser:
    ...
    -------
    list
        The imported :class:`~model_story.models.Run` objects (one per sub-directory).
    """
    logdir = Path(logdir)
    runs = []
...
def _import_one(event_dir: Path, *, name: str, port: int) -> Any:  # noqa: ANN401
    """Import a single TensorBoard log directory into model-story."""
