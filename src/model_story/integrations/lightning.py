...
The callback wraps :class:`~model_story.sdk.live_reporter.LiveReporter`
...
module imports it lazily so that importing ``model_story.integrations.lightning``
...
class StoryCallback:
    """Report training metrics to model-story via
    :class:`~model_story.sdk.live_reporter.LiveReporter`.

    Parameters
    ----------
    task:
        Task type hint (e.g. ``"classification"``, ``"detection"``).
        Passed directly to :class:`~model_story.sdk.live_reporter.LiveReporter`.
    primary_metric:
