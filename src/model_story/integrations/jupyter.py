...
Registers the ``%model_story`` and ``%%model_story`` magics.
...
Usage in a notebook cell::
    ...
    # Live mode — runs the next cell's output through model-story:
    %%model_story --live
    !python train.py

The magic renders an ``<iframe>`` pointing at the local model-story server.
...
def load_ipython_extension(ipython: Any) -> None:  # noqa: ANN401
    """Called by ``%load_ext model_story``."""
    ipython.register_magic_function(_line_magic, magic_kind="line", magic_name="model_story")
    ipython.register_magic_function(_cell_magic, magic_kind="cell", magic_name="model_story")


def unload_ipython_extension(ipython: Any) -> None:  # noqa: ANN401
    """Called by ``%unload_ext model_story``."""


# ── magic implementations ─────────────────────────────────────────────────────

def _line_magic(line: str) -> None:
    """``%model_story [OPTIONS] [LOG_FILE]``
    ...
    try:
        ...
        print("[model-story] IPython is required for the magic.")
        return
...
def _cell_magic(line: str, cell: str) -> None:
    ...
    Runs the cell body and pipes its stdout through model-story in live mode.
    ...
    # Write cell to a temp script and run it, piping stdout to model-story
    import os
    import tempfile
    ...
    try:
        ...
        try:
            ...
            print(f"[model-story] Dashboard: {url}")
...
def _ensure_server(*, port: int = 7860) -> None:
    """Start the model-story server if not already running on *port*."""
    with _server_lock:
...
def _parse_and_register(log_path: Path, *, task: str | None, port: int) -> Any:  # noqa: ANN401
    ...
    from model_story.sdk.parse import parse
...
def _make_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="%model_story", add_help=False)
