"""Loader for .model-story.yaml grade threshold configuration.

Users can place a ``.model-story.yaml`` in their project root (or
``~/.model-story/.model-story.yaml`` for per-user defaults) to override
the built-in grade thresholds that model-story uses when scoring runs.

Search order when no explicit path is given:

1. Current working directory.
2. Each parent directory, up to the filesystem root.
3. ``~/.model-story/.model-story.yaml``.

Example ``.model-story.yaml``::
...
_CONFIG_FILENAME = ".model-story.yaml"


@dataclass
class GradeConfig:
    """Parsed grade threshold configuration from ``.model-story.yaml``."""
...
def find_config_file(start: Path | None = None) -> Path | None:
    """Locate the nearest ``.model-story.yaml`` file.

    Walks up from *start* (defaults to ``Path.cwd()``), then checks
    ``~/.model-story/.model-story.yaml``.  Returns the first path found,
    or ``None``.
    """
    ...
    # Per-user fallback
    home_config = Path.home() / ".model-story" / _CONFIG_FILENAME
