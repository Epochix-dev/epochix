...
_CONV_ENCODER: _LayerMeta = {"tech_label": "ENCODER", "plain_label": "Feature extractor", "visual_type": "conv"}
_VIS_XFORMER: _LayerMeta = {"tech_label": "ENCODER", "plain_label": "Vision transformer", "visual_type": "attention"}
...
_TYPE_MAP: list[tuple[str, _LayerMeta]] = [
    ...
    ("depthwiseconv", {"tech_label": "CONV",   "plain_label": "Spatial patterns",   "visual_type": "conv"}),
    ("separableconv", {"tech_label": "CONV",   "plain_label": "Spatial patterns",   "visual_type": "conv"}),
    ...
    # ── Recurrent ──────────────────────────────────────────────────────────
    ("bilstm",        {"tech_label": "MEMORY", "plain_label": "Bi-directional memory", "visual_type": "recurrent"}),
    ("bigru",         {"tech_label": "MEMORY", "plain_label": "Bi-directional memory", "visual_type": "recurrent"}),
    ...
    ("multiheadattention", {"tech_label": "FOCUS", "plain_label": "Multi-head attention", "visual_type": "attention"}),
    ("multiheadattn", {"tech_label": "FOCUS",  "plain_label": "Multi-head attention","visual_type": "attention"}),
    ("crossattention",{"tech_label": "FOCUS",  "plain_label": "Cross attention",    "visual_type": "attention"}),
    ("selfattention", {"tech_label": "FOCUS",  "plain_label": "Self attention",     "visual_type": "attention"}),
    ("attention",     {"tech_label": "FOCUS",  "plain_label": "Reads full context", "visual_type": "attention"}),
    ("transformerencoder", {"tech_label": "FOCUS", "plain_label": "Transformer encoder", "visual_type": "attention"}),
    ("transformerdecoder", {"tech_label": "FOCUS", "plain_label": "Transformer decoder", "visual_type": "attention"}),
    ("transformer",   {"tech_label": "FOCUS",  "plain_label": "Transformer",        "visual_type": "attention"}),
    ("distilbert",    {"tech_label": "FOCUS",  "plain_label": "Language understander","visual_type": "attention"}),
    ("roberta",       {"tech_label": "FOCUS",  "plain_label": "Language understander","visual_type": "attention"}),
    ("deberta",       {"tech_label": "FOCUS",  "plain_label": "Language understander","visual_type": "attention"}),
    ("electra",       {"tech_label": "FOCUS",  "plain_label": "Language understander","visual_type": "attention"}),
    ("albert",        {"tech_label": "FOCUS",  "plain_label": "Language understander","visual_type": "attention"}),
    ("bert",          {"tech_label": "FOCUS",  "plain_label": "Language understander","visual_type": "attention"}),
    ("llama",         {"tech_label": "FOCUS",  "plain_label": "Language generator",  "visual_type": "attention"}),
    ("mistral",       {"tech_label": "FOCUS",  "plain_label": "Language generator",  "visual_type": "attention"}),
    ("qwen",          {"tech_label": "FOCUS",  "plain_label": "Language generator",  "visual_type": "attention"}),
    ("gpt",           {"tech_label": "FOCUS",  "plain_label": "Language generator",  "visual_type": "attention"}),
    ("t5",            {"tech_label": "FOCUS",  "plain_label": "Seq2seq transformer", "visual_type": "attention"}),
    ("bart",          {"tech_label": "FOCUS",  "plain_label": "Seq2seq transformer", "visual_type": "attention"}),
    ("encoderlayer",  {"tech_label": "FOCUS",  "plain_label": "Transformer block",  "visual_type": "attention"}),
    ("decoderlayer",  {"tech_label": "FOCUS",  "plain_label": "Transformer block",  "visual_type": "attention"}),
    # ── Embeddings ─────────────────────────────────────────────────────────
    ("patchembed",    {"tech_label": "EMBED",  "plain_label": "Patch embedder",     "visual_type": "dense"}),
    ("positionalencoding", {"tech_label": "EMBED", "plain_label": "Position encoder", "visual_type": "dense"}),
    ("posencoding",   {"tech_label": "EMBED",  "plain_label": "Position encoder",   "visual_type": "dense"}),
...
_TRIVIAL = (
    "flatten", "identity", "reshape", "permute", "view", "relu", "gelu", "silu",
    "swish", "sigmoid", "tanh", "softmax", "leakyrelu", "elu", "mish", "hardswish",
    "hardtanh", "prelu", "activation",
)
...
def _make_layer(idx: int, name: str, layer_type: str, params_str: str) -> ArchLayer:
    ...
    return ArchLayer(idx=idx, name=name, layer_type=layer_type, params=params,
                     params_str=params_str.strip(), **_classify(layer_type))
...
def _parse_pytorch_lightning(lines: list[str]) -> list[ArchLayer]:
    ...
    for line in lines:
        ...
        if m:
            result.append(_make_layer(int(m.group(1)), m.group(2).strip(),
...
def _parse_torchinfo(lines: list[str]) -> list[ArchLayer]:
    ...
    for raw in lines:
        ...
        if not line or set(line) <= set("=_-─ "):
            continue
        if re.match(r"^(Total params|Trainable params|Non-trainable|Estimated)", line, re.IGNORECASE):
            if result:
                break
            continue
        m = _TI_ROW.match(raw)
...
_ULTRA_ROW = re.compile(
    r"^\s*(\d+)\s+(?:-?\d+|\[[\d,\s]+\])\s+\d+\s+(\d+)\s+([\w.]+)"
)
...
_SUMMARY_LINE = re.compile(
    r"([A-Za-z][\w/+-]*)\s+summary:.*?([\d,]+)\s+param", re.IGNORECASE
)
