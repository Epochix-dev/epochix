...
from epochix.store.ring_buffer import RingBuffer
...
class Hub:
    ...
    A :class:`~epochix.store.ring_buffer.RingBuffer` per run enables
    ...
    def make_message(
        *,
        ...
        """Construct a :class:`~epochix.models.WSMessage` with *now* timestamp."""
        return WSMessage(
            v=1,
            type=msg_type,
            run_id=run_id,
