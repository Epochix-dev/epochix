# Changelog — Model Learning Story (VS Code Extension)
...
- **VS Code postMessage bridge** in the shared frontend — gated on
  `window.__MS_VSCODE__`. Standalone mode now receives `init` / `frame` /
...
The webview renders the shared model-story dashboard, so every fix in the
...
markers), `model-story demo` onboarding, real-YOLO end-to-end verification.
...
- Sidecar mode — auto-discovers the Python `model-story` package and
